...
def make_env(steps):
    #defined two task categories(long and short schedules) 
    ...
    generators = [
        # rare but large
        TaskGenerator(long_cat,  generator_seed=10, probability=0.05),  
        
        # frequent and tiny 
        TaskGenerator(short_cat, generator_seed=11, probability=0.3),   
    ]
...
def plot_pred_vs_true(logger: Log, filename: str):
    ...
    #check to see if correcly estimating predicted lengths of tasks
...
def run(model_path, H, steps, out_dir):
    ...
    #calls evaluate policy with before and afters
